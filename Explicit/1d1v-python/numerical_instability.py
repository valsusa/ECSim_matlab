...
#L=1200. # Domain size - finite grid instability

DT = 0.1 # Time step

#DT = 2.0 # Time step instability

TOT_Time = 100.  # For testing finite grid instability

NT = int(TOT_Time / DT)  # Number of time steps

TOut = round(NT/20.) # Output period
...
print 'Debye condition', dx/(VT/WP)
...
for it in xrange(NT+1):
    ...
    if (np.mod(it, TOut) == 0) and verbose:
        ...
        plt.bar(L*k, ft, label='fft(E)', linewidth=2)
